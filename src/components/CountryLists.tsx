...
const CountryLists: React.FC = () => {
	...
	const getArea = (countryName: string): number => {
	...
	const lastIndex: number = presentPage * eachPage;
	const firstIndex: number = lastIndex - eachPage;
	const paginatedCountries: Country[] = filteredCountries.slice(
		firstIndex,
		lastIndex
	);
	const pageCount: number = Math.ceil(filteredCountries.length / eachPage);
	const numbers: number[] = [...Array(pageCount + 1).keys()].slice(1);

	const prePage = () => {
		if (presentPage !== firstIndex) setPresentPage(presentPage - 1);
	};
	const changePage = (c: number) => {
		setPresentPage(c);
	};
	const nextPage = () => {
		if (presentPage !== lastIndex) setPresentPage(presentPage + 1);
	};
	const active = (pageNumber: number, currentPage: number) => {
